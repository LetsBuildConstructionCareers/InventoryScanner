...
class AddItemsToVehicleLandingActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val scannerLauncher = scannerForNewActivity(this, AddItemsToVehicleActivity::class.java)
        setContent {
...
class AddItemsToVehicleActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val containerId = intent.getStringExtra("barcode_id") ?: return
        val intent = Intent(this, AddSingleItemToVehicleActivity::class.java)
        ...
        val scannerForAddSingleItem = registerForActivityResult(ScanContract()) { scannedBarcode: ScanIntentResult ->
            run {
                if (scannedBarcode.contents == null) {
                    Toast.makeText(this@AddItemsToVehicleActivity, "Cancelled", Toast.LENGTH_LONG).show()
                } else {
                        ...
                        override fun onResponse(call: Call<String>, response: Response<String>) {
                            ...
                            if (response.isSuccessful && response.body() != null) {
                                ...
                                if (itemParent.isNotBlank()) {
                                    Toast.makeText(this@AddItemsToVehicleActivity, "Item is already in a container!!", Toast.LENGTH_LONG).show()
                                } else {
...
class AddSingleItemToVehicleActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        ...
        setContent {
            AddSingleItemToContainerUI(itemId, containerId, this,
                {inventoryApi, containerId, itemIds -> inventoryApi.addItemsToVehicle(containerId, itemIds)})
        }
    }
}